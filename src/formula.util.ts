...
export function tokenize(formula: string): Token[] {
  ...
  while (index < formula.length) {
    ...
    if (/[0-9]/.test(char) || (char === '.' && /[0-9]/.test(formula[index + 1] ?? ''))) {
      let end = index;
    ...
    if (/[A-Za-z_]/.test(char)) {
      ...
      if (!FUNCTIONS[name]) {
        throw new FormulaError(
          `Fonction inconnue : « ${name} ». Fonctions disponibles : ${AVAILABLE_FUNCTIONS.join(', ')}.`,
        );
      }
...
/* ============================================================================================
 * 2. ANALYSE SYNTAXIQUE — descente récursive
 * ========================================================================================== */

type Node =
  | { kind: 'number'; value: number }
  | { kind: 'variable'; reference: string }
...
class Parser {
  ...
  parse(): Node {
    ...
    if (this.position < this.tokens.length) {
      throw new FormulaError(
        `Élément inattendu après la fin de l'expression : « ${this.tokens[this.position].value} ».`,
      );
    }
    return node;
  }
    ...
    switch (token.type) {
      ...
      case 'function': {
        ...
        if (args.length < signature.minArgs || args.length > signature.maxArgs) {
          throw new FormulaError(
            `« ${token.value} » attend entre ${signature.minArgs} et ${signature.maxArgs} argument(s), ` +
...
export function extractNumbers(raw: unknown): number[] {
  ...
  if (Array.isArray(raw)) return raw.flatMap((entry) => extractNumbers(entry));

  if (typeof raw === 'object') {
    return Object.values(raw as Record<string, unknown>).flatMap((entry) => extractNumbers(entry));
  }
...
function countAnswered(raw: unknown): number {
  ...
  if (Array.isArray(raw)) return raw.filter((entry) => countAnswered(entry) > 0).length;
  ...
  if (typeof raw === 'object') {
    return Object.values(raw as Record<string, unknown>).filter((entry) => countAnswered(entry) > 0)
      .length;
  }
...
function collectValues(node: Node, scope: FormulaScope): number[] {
...
function evaluateNode(node: Node, scope: FormulaScope): number {
  switch (node.kind) {
    case 'number':
      return node.value;
    ...
    case 'binary': {
      ...
      switch (node.operator) {
        ...
        case '/':
          ...
          if (right === 0) throw new FormulaError('Division par zéro.');
          return left / right;
        case '%':
          if (right === 0) throw new FormulaError('Modulo par zéro.');
          return left % right;
      ...
      if (node.name === 'COUNT') {
        return node.args.reduce((total, arg) => {
          if (arg.kind === 'variable') {
            if (!(arg.reference in scope)) throw new PendingValueError(arg.reference);
 ...
 */
export function parseFormula(formula: string): { references: string[] } {
 ...
 */
export function evaluateFormula(formula: string, scope: FormulaScope): FormulaResult {
  try {
    const tokens = tokenize(formula);
