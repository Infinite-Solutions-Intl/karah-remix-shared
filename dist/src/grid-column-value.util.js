...
exports.findDuplicateColumnValues = findDuplicateColumnValues;
exports.findDuplicateColumnLabels = findDuplicateColumnLabels;
exports.canSelectMore = canSelectMore;
/**
 ...
 */
function resolveColumnValue(column) {
    var _a;
    ...
    if (label.length > 0)
        return label;
    return (_a = column.id) !== null && _a !== void 0 ? _a : '';
}
/**
 ...
 */
function findDuplicateColumnValues(columns) {
    const seen = new Set();
...
function findDuplicateColumnLabels(columns) {
    var _a;
    const seen = new Set();
    const duplicates = new Set();
    for (const column of columns) {
        const label = ((_a = column.label) !== null && _a !== void 0 ? _a : '').trim().toLowerCase();
        if (label.length === 0)
            continue;
        if (seen.has(label))
 ...
 */
function canSelectMore(currentSelection, maxPerRow) {
    if (maxPerRow === undefined || maxPerRow === null)
        return true;
