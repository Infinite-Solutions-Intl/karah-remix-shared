...
exports.MAX_USABLE_ACCURACY_METERS = void 0;
 ...
 */
exports.MAX_USABLE_ACCURACY_METERS = 50;
 ...
 */
function distanceInMeters(fromLat, fromLon, toLat, toLon) {
 ...
 */
function evaluatePosition(position, site) {
    ...
    if (typeof accuracy === 'number' && accuracy > exports.MAX_USABLE_ACCURACY_METERS) {
    ...
    const distance = distanceInMeters(position.latitude, position.longitude, site.latitude, site.longitude);
 ...
 */
function describeVerdict(verdict, siteName) {
    switch (verdict.flag) {
        case 'OUT_OF_ZONE':
            return (`Vous n'êtes pas dans le périmètre${siteName ? ` du site « ${siteName} »` : ''}` +
        ...
        case 'NO_POSITION':
            return ("Votre position n'a pas pu être déterminée. Vous pouvez continuer, la collecte sera " +
                'signalée comme non vérifiée.');
        case 'LOW_ACCURACY':
            return (`Le signal GPS est trop imprécis (plus de ${exports.MAX_USABLE_ACCURACY_METERS} m). ` +
