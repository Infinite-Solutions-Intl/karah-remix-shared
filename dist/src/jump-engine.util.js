...
exports.buildQuestionPositions = buildQuestionPositions;
exports.findJumpOrigin = findJumpOrigin;
exports.computeSkippedBlocks = computeSkippedBlocks;
exports.collectOptionalQuestions = collectOptionalQuestions;
...
function buildQuestionPositions(blocks) {
 ...
 */
function findJumpOrigin(rule, positions) {
    var _a;
    let latest = null;
    for (const condition of (_a = rule.conditions) !== null && _a !== void 0 ? _a : []) {
        if (!condition.questionId)
            continue;
        const position = positions.get(condition.questionId);
        if (!position)
            continue;
        if (!latest || comparePositions(position, latest) > 0)
            latest = position;
    }
    return latest;
}
 ...
 */
function computeSkippedBlocks(blocks, activeJumps, positions) {
 ...
 */
function resolveTargetPosition(rule, blocks, positions) {
    var _a;
    if (rule.targetType === 'QUESTION') {
        return (_a = positions.get(rule.targetId)) !== null && _a !== void 0 ? _a : null;
    }
 ...
 */
function collectOptionalQuestions(blocks, hiddenBlockIds) {
    return blocks
