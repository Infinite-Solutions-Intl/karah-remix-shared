...
exports.OTHER_ROW_LABEL_KEY = void 0;
exports.isOtherRow = isOtherRow;
exports.readOtherLabel = readOtherLabel;
exports.writeOtherLabel = writeOtherLabel;
exports.stripOtherLabel = stripOtherLabel;
exports.resolveRowLabel = resolveRowLabel;
exports.isOtherRowIncomplete = isOtherRowIncomplete;
/** Clé sous laquelle le libellé saisi par le répondant est enregistré. */
exports.OTHER_ROW_LABEL_KEY = '__label';
 ...
 */
function readOtherLabel(rawRow) {
    if (!rawRow || typeof rawRow !== 'object' || Array.isArray(rawRow))
        return '';
    const value = rawRow[exports.OTHER_ROW_LABEL_KEY];
 ...
 */
function writeOtherLabel(rawRow, label) {
    const row = rawRow && typeof rawRow === 'object' && !Array.isArray(rawRow)
        ? { ...rawRow }
        : {};
    if (label.trim().length === 0)
        delete row[exports.OTHER_ROW_LABEL_KEY];
    else
        row[exports.OTHER_ROW_LABEL_KEY] = label;
    return row;
}
/** Contenu d'une ligne débarrassé du libellé « Autre », pour ne traiter que les colonnes. */
function stripOtherLabel(rawRow) {
    if (!rawRow || typeof rawRow !== 'object' || Array.isArray(rawRow))
        return {};
    const { [exports.OTHER_ROW_LABEL_KEY]: _label, ...rest } = rawRow;
    return rest;
}
/**
 ...
 */
function resolveRowLabel(row, rawRow) {
    var _a;
    if (isOtherRow(row)) {
 ...
 * laquelle.
 */
function isOtherRowIncomplete(row, rawRow) {
    if (!isOtherRow(row))
        return false;
