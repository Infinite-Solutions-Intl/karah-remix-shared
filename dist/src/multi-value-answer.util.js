...
exports.extractSelectedValues = extractSelectedValues;
exports.answerContains = answerContains;
exports.answerIsIn = answerIsIn;
exports.answerEquals = answerEquals;
/**
 ...
 */
function extractSelectedValues(rawAnswer) {
    ...
    if (typeof rawAnswer === 'object') {
        ...
        return Object.values(rawAnswer).flatMap((entry) => extractSelectedValues(entry));
    }
 ...
 */
function answerContains(rawAnswer, expected) {
 ...
 */
function isStructuredAnswer(rawAnswer) {
    if (Array.isArray(rawAnswer) || (rawAnswer !== null && typeof rawAnswer === 'object'))
        return true;
...
function answerIsIn(rawAnswer, expected) {
 ...
 */
function answerEquals(rawAnswer, expected) {
    if (expected === null || expected === undefined)
        return false;
    const target = String(expected).trim().toLowerCase();
    const selected = extractSelectedValues(rawAnswer).map((entry) => entry.trim().toLowerCase());
